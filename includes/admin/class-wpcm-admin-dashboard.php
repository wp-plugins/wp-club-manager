...
if ( ! class_exists( 'WPCM_Admin_Dashboard' ) ) :
...
class WPCM_Admin_Dashboard {
	...
	public function init() {
		...
		wp_add_dashboard_widget( 'wpclubmanager_dashboard_upcoming', __( 'This Week\'s Upcoming Matches', 'wpclubmanager' ), array( $this, 'upcoming_matches_widget' ) );
	...
	public function glance_items( $items ) {
        ...
        foreach ( $post_types as $type ):
			...
			if ( $num_posts && $num_posts->publish ) {
				...
				$text = _n( '%s ' . $post_type->labels->singular_name, '%s ' . $post_type->labels->name, $num_posts->publish, 'wpclubmanager' );
	...
	public function upcoming_matches_widget() {
		...
		if ( $matches ) {
			...
			<li><?php _e('No upcoming matches.', 'wpclubmanager'); ?></li>
		...
		<div class="add-new-match-link">
			<a class="button btn" href="post-new.php?post_type=wpcm_match"><?php _e( 'Add New Match', 'wpclubmanager' ); ?></a>
		</div>
...
return new WPCM_Admin_Dashboard();