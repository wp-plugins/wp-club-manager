	...
	class WPCM_Importer extends WP_Importer {
		...
		function dropdown( $selected ) {
			...
			<select name="wpcm_columns[]" data-index="<?php echo array_search( $selected, array_keys( $this->columns ) ); ?>">
				<option value="0">&mdash; <?php _e( 'Disable', 'wpclubmanager' ); ?> &mdash;</option>
		...
		function import_table( $file ) {
			...
			if ( ! is_file($file) ):
				...
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'wpclubmanager' ) . '</strong><br />';
				echo __( 'The file does not exist, please try again.', 'wpclubmanager' ) . '</p>';
			...
			if ( ( $handle = fopen( $file, "r" ) ) !== FALSE ):
				...
				if ( sizeof( $header ) >= 1 ):
					...
					<form enctype="multipart/form-data" id="import-upload-form" class="wpcm-form" method="post" action="<?php echo esc_attr(wp_nonce_url($action, 'import-upload')); ?>">
						...
						<p class="alignright">
							<?php printf( __( 'Displaying %s&#8211;%s of %s', 'wpclubmanager' ), 1, $this->imported+1, $this->imported+1 ); ?>
						</p>
						<p class="submit">
					...
					echo '<p><strong>' . __( 'Sorry, there has been an error.', 'wpclubmanager' ) . '</strong><br />';
					echo __( 'The CSV is invalid.', 'wpclubmanager' ) . '</p>';
					$this->footer();
					die();
		...
		function handle_upload() {
			...
			if ( empty( $_POST['file_url'] ) ) {
				...
				if ( file_exists( ABSPATH . $_POST['file_url'] ) ) {
					...
					echo '<p><strong>' . __( 'Sorry, there has been an error.', 'wpclubmanager' ) . '</strong></p>';
					return false;
