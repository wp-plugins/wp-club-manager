...
class WPCM_Admin_CPT_Sponsor extends WPCM_Admin_CPT {
	...
	public function enter_title_here( $text, $post ) {
		...
		if ( $post->post_type == 'wpcm_sponsor' )
			return __( 'Enter sponsor name', 'wpclubmanager' );
	...
	public function custom_admin_post_thumbnail_html( $content ) {
	    ...
	    if( 'wpcm_sponsor' == $current_screen->post_type ) {
	        ...
	        $content = str_replace( __( 'Set featured image' ), __( 'Set sponsor logo', 'wpclubmanager' ), $content);
	    	$content = str_replace( __( 'Remove featured image' ), __( 'Remove sponsor logo', 'wpclubmanager' ), $content );
	    }
