...
class WPCM_License {
	...
	private function includes() {
		if ( ! class_exists( 'WPCM_Plugin_Updater' ) ) require_once 'WPCM_Plugin_Updater.php';
	}
	...
	public function settings( $settings ) {
		$wpcm_license_settings = array(
			array(
				...
				'name'    => sprintf( __( '%1$s License Key', 'wpclubmanager' ), $this->item_name ),
				'desc'    => '',
				'type'    => 'license_key',
	...
	public function activate_license() {
		...
		if( ! wp_verify_nonce( $_REQUEST[ $this->item_shortname . '_license_key-nonce'], $this->item_shortname . '_license_key-nonce' ) ) {
			...
			wp_die( __( 'Nonce verification failed', 'wpclubmanager' ), __( 'Error', 'wpclubmanager' ), array( 'response' => 403 ) );
		...
		if( ! empty( $license_error->error ) ) {
			...
			switch( $license_error->error ) {
				...
				case 'item_name_mismatch' :
					...
					$message = __( 'This license does not belong to the product you have entered it for.', 'wpclubmanager' );
					break;

				case 'no_activations_left' :

					$message = __( 'This license does not have any activations left', 'wpclubmanager' );
					break;

				case 'expired' :

					$message = __( 'This license key is expired. Please renew it.', 'wpclubmanager' );
					break;

				default :

					$message = sprintf( __( 'There was a problem activating your license key, please try again or contact support. Error code: %s', 'wpclubmanager' ), $license_error->error );
					break;
