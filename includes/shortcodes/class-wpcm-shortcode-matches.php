...
class WPCM_Shortcode_Matches {
	...
	public static function output( $atts ) {
		...
		$title 		= ( isset( $atts['title'] ) ? $atts['title'] : __( 'Fixtures & Results', 'wpclubmanager' ));
		...
		if( $matches ) {
			...
			<?php } else { ?>
				...
				<thead>
					...
					<tr>
						<th class="wpcm-date"><?php _e('Date', 'wpclubmanager'); ?></th>
						<th class="venue"><?php _e('Venue', 'wpclubmanager'); ?></th>
						<?php if ( $thumb == '1' ) { ?>
							<th class="club-thumb">&nbsp;</th>
						<?php } ?>
						<th class="opponent"><?php _e('Opponent', 'wpclubmanager'); ?></th>
						<th class="competition"><?php _e('Competition', 'wpclubmanager'); ?></th>
						<th class="result"><?php _e('Result', 'wpclubmanager'); ?></th>
					</tr>
			...
			<p><?php _e('No matches played yet.', 'wpclubmanager'); ?></p>
		...
		if ( $matches ) {
			...
			foreach( $matches as $match ) {
				...
				if ( $type == '1' && $club == $home_club ) {
					...
					if ( $neutral ) {
						$venue = __('N', 'wpclubmanager');
					} else {
						$venue = __('H', 'wpclubmanager');
					}
